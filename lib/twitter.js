...
Twitter.prototype.follow = function (keyword, reconnect) {
  reconnect = typeof reconnect === 'undefined' || reconnect

  if (this._following[keyword]) {
    this._following[keyword]++
  } else {
    this._following[keyword] = 1
    this.stale = true
    if (reconnect) {
      this.reconnect()
    }
...
Twitter.prototype.following = function () {
  return Object.keys(this._following)
}
...
Twitter.prototype.unfollow = function (keyword, reconnect) {
  reconnect = typeof reconnect === 'undefined' || reconnect

  if (typeof this._following[keyword] === 'undefined') {
    return
  }
  if (--this._following[keyword] === 0) {
    delete this._following[keyword]
    this.stale = true
    if (reconnect) {
      this.reconnect()
    }
...
Twitter.prototype.connect = function () {
  this.stale = false
  if (Object.keys(this._tracking).length === 0 && Object.keys(this._following).length === 0 ) {
    return
  }
  ...
  this.stream = request.post({
    ...
    form: {
      ...
      follow: Object.keys(this._following).join(',')
    }
  })
